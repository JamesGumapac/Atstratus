    ...
    function (url, currentRecord, format, search, record, https) {
        ...
        function pageInit(context) {
            ...
            if (window.location.href.indexOf('subsidiary') != -1) {
                var geneRateCSVButton = context.currentRecord.getField({
                    fieldId:  'custpage_generate_csv'
                })
                var property = context.currentRecord.getField({
                    fieldId: 'custpage_location_filter'
                })
                geneRateCSVButton.isDisabled = false
                ...
                geneRateCSVButton.isDisabled = true
            ...
            if (window.location.href.indexOf('startdate') != -1) {
                context.currentRecord.setValue({
                    ...
                    value: parseDateToShortDate(JSON.parse(decodeURIComponent(urlParams.get('startdate')))),
                    ignoreFieldChange: true
                });
            }
            ...
            if (window.location.href.indexOf('enddate') != -1) {
                ...
                context.currentRecord.setValue({
                    ...
                    value: parseDateToShortDate(JSON.parse(decodeURIComponent(urlParams.get('enddate')))),
                    ignoreFieldChange: true
                });
            }
        ...
        function generateReportToCSV() {
            ...
            //console.log(JSON.stringify(getFieldsForProcessing['consolidateSubProp']))
            ...
            if (retrieveSearchCount == 0) {
                ...
                alert('File will be sent to your email address')
                // Call Suitelet to generate task

                console.log('fieldsforProcessing :' + JSON.stringify(getFieldsForProcessing));
                var currInstanceChecker = search.load({id:'customsearch520'});
                ...
                if(checkInstCount){
                    ...
                    var responseContainer = resolveScriptWithParameters('', 'TRIGGERMRVIASLET', getFieldsForProcessing)
                    console.log(JSON.stringify(responseContainer));
                    return null;
                }
            ...
            retrieveSearchResults.each(function (resultHolder) {
                ...
                var units = resultHolder.getText({
                    name: 'unit',
                    summary: 'GROUP'
            ...
            mainCSVtoRender = 'COGS / EXPENSE ACCOUNT, Subsidiary ,Property Name, Vehicle Number, Transaction Date, Transaction Reference, Item, Item Class, Quantity, Units of Measure, Sum of Item Cost, Sum of Amount\r\n' + mainCSVtoRender;
        ...
        function runMainCogsSearch(searchId, paramsToProcess) {
            ...
            alert('COGS Total Number rows = ' + JSON.stringify(checkCount));
        ...
        function parseDateToShortDate(dateString) {


            var dateSplit = dateString.split('/');
            var returnDateVal = new Date(dateSplit[1] + '/' + dateSplit[0] + '/' + dateSplit[2]);
