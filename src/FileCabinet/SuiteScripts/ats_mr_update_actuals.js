...
define(['N/record', 'N/search'],
    ...
    (record, search) => {
        ...
        const ACCOUNTINGPERIODSEARCHSTATISTIC = 'customsearch_ats_mis_sta_period_search'
        const MISSTAINVOICESEARCH = 'customsearch_final_ats_sta_actuals'
        /**
        ...
        const getInputData = (inputContext) => {
            var mainprortySearch = search.load({
                id: ACCOUNTINGPERIODSEARCHSTATISTIC
            })
        ...
        const map = (context) => {
            ...
            if (searchResultCount > 0) {
                ...
                try {
                    ...
                    var revPorActualTotal = revPorActual/roomSoldActual
                    var isInfinityRevPor =  isFinite(revPorActualTotal)
                    if(revPorActualTotal > 0 && isInfinityRevPor == true){
                       log.audit('aa revPor Actual ' + id, Math.ceil(revPorActualTotal) )
                        misRec.setValue({
                            fieldId: 'custrecord291',
                            value: Math.ceil(revPorActualTotal)
                        })
                    }
